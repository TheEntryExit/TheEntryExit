...
function isValidStep(step, index) {
  ...
  // C1 has no previous candle in sequence, so interaction fields must be Ignore.
  if (index === 0) {
    return step.wickInteraction === 'Ignore' && step.closePosition === 'Ignore';
  }
...
function analyzeSequence(sequence) {
  ...
  for (let i = 0; i <= candles.length - sequence.length - 1; i += 1) {
    ...
    for (let seqIndex = 0; seqIndex < sequence.length; seqIndex += 1) {
      ...
      if (seqIndex > 0) {
        const previousInSequence = candles[i + seqIndex - 1];
  ...
  for (let i = 0; i < sequence.length; i += 1) {
    if (!isValidStep(sequence[i], i)) {
      return res.status(400).json({
        error: i === 0
          ? 'C1 must use Wick interaction = Ignore and Close position = Ignore'
          : `Invalid candle rule at index ${i}`
      });
    }
  }
